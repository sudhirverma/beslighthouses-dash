...
import { Button, Card, Container, Paper, Stack, Table, TableBody, TableCell, TableContainer, TablePagination, TableRow, Typography } from "@mui/material";
...
export default function UserPage() {
    ...
    const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - getUSERLIST.length) : 0;

    const filteredUsers = applySortFilter(getUSERLIST, getComparator(order, orderBy), filterName);
                ...
                <Card>
                    ...
                    <TableContainer sx={{ minWidth: 800, color: 'red' }}>
                        <Table>
                            ...
                            <TableBody>
                                {
                                    filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row: { id: any; name: string; description: any; bes_technology_stack: any; html_url: string; license: any; }) => {
                                        const { id, name, description, bes_technology_stack, license, html_url } = row;
                                        let licenseName;
                                        if (license) {
                                                ...
                                                <TableCell component="th" scope="row" padding="none">
                                                    <Stack direction="row" alignItems="center" spacing={2}>
                                                        <Typography sx={{ position: 'relative', left: "16px" }} variant="subtitle2" noWrap>
                                                            <Button onClick={() => openGithubLink(html_url)}> {id} </Button>
                                                        </Typography>
                                                    </Stack>
                                                </TableCell>
